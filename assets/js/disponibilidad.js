...
function deleteAllData() {
    const confirmDelete = confirm('¿Estás seguro de que deseas borrar toda la disponibilidad guardada?');
    if (confirmDelete) {
...
function removeDay(day) {
    const confirmRemove = confirm(`¿Estás seguro de que deseas eliminar el día ${day}?`);
    if (confirmRemove) {
...
function populateAvailabilityTable(day, availabilityTemplate) {
    ...
    const hourCells = document.querySelectorAll(`#availability-${day} .hour-cell`);
    hourCells.forEach(cell => {
        const time = cell.textContent;
        cell.addEventListener('mouseover', () => {
            const cells = document.querySelectorAll(`#availability-${day} [data-time="${time}"]`);
        ...
        cell.addEventListener('click', () => {
            const cells = document.querySelectorAll(`#availability-${day} [data-time="${time}"]`);
            const isSelected = Array.from(cells).some(c => c.classList.contains('selected'));
            cells.forEach(c => {
                c.classList.toggle('selected', !isSelected);
                updateAvailabilityData(day, time, c.getAttribute('data-pista'), !isSelected);
            });
        });
    });

    // Añadir event listener para las celdas de pista
    const pistaCells = document.querySelectorAll(`#availability-${day} thead th:not(:first-child)`);
    pistaCells.forEach((cell, index) => {
        const pista = `Pista ${index + 1}`;
        cell.addEventListener('mouseover', () => {
            const cells = document.querySelectorAll(`#availability-${day} [data-pista="${pista}"]`);
            cells.forEach(c => c.classList.add('preview'));
        });
        cell.addEventListener('mouseout', () => {
            const cells = document.querySelectorAll(`#availability-${day} [data-pista="${pista}"]`);
        ...
        cell.addEventListener('click', () => {
            ...
            cells.forEach(c => {
                c.classList.toggle('selected', !isSelected);
                updateAvailabilityData(day, c.getAttribute('data-time'), pista, !isSelected);
            });
        });
    });
}
